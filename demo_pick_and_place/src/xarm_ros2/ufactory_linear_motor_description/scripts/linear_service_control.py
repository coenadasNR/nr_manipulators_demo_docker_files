...
class LinearMotorNode(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(MoveLinearMotor, 'move_linear_motor', self.move_service_callback)
    ...
    def timer_callback(self):
        ...
        if code == 0:
            ...
            msg.position = [pos_mm / 1000.0]  # mm to m
    ...
    def cmd_callback(self, msg: JointState):
        if 'slider_joint' in msg.name:
            ...
            target_m = max(0.0, min(0.7, target_m))  # Clamp
            target_mm = target_m * 1000.0
            code = self.arm.set_linear_track_pos(target_mm, wait=True)
            if code != 0:
                self.get_logger().error(f"Failed to set track to {target_mm} mm (requested {target_m} m), code={code}")
        else:
            self.get_logger().warn("Received JointState without 'slider_joint'")

    # ADDED
    def move_service_callback(self, request, response):
        target_m = max(0.0, min(0.7, request.target_position_m))
        target_mm = target_m * 1000.0
        code = self.arm.set_linear_track_pos(target_mm, wait=True)

        if code == 0:
            response.success = True
            response.message = f"Moved successfully to {target_m:.3f} m."
        else:
            response.success = False
            response.message = f"Failed to move to {target_m:.3f} m. Error code: {code}"
